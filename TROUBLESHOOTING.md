...
1. 在扩展管理页面，点击"FigClip"下的"检查视图" → "service worker"
2. 查看是否有错误信息
3. 如果有错误，请记录错误信息
...
2. 按 F12 打开开发者工具
3. 切换到 Console 标签
4. 应该能看到 `[FigClip] 脚本文件开始加载...` 的日志
5. 如果没有，说明content script没有注入

### 步骤4：手动检查文件

在扩展管理页面：
1. 点击"FigClip"下的"检查视图" → "service worker"
2. 在控制台中运行：
```javascript
...
确认manifest.json中的配置：
- `content_scripts` 中的 `matches` 包含正确的目标网址
- `js` 数组中包含 `"content.js"`
- 文件路径正确
...
扩展根据页面 DOM 和文案判断每条内容是「帖子」「回复」「转发」还是「引用」，网站改版或界面语言不同可能导致误判。

### 若类型识别错误，反馈时请提供：

- **页面场景**：时间线 / 详情页 / 用户主页 / 搜索页等
- **内容类型**：您认为该条应是「帖子 / 回复 / 转发 / 引用」中的哪一种
- **界面语言**：当前使用的语言（如中文、English）

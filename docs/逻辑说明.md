...
本文档整合扩展的两大判断逻辑：**内容类型识别**（Tweet / Reply / Repost / Quote）与 **保存按钮添加**，并与 [content.js](../content.js) 实现一致。
...
- **操作**：按住 **Shift** 连续点击多条推文的**收藏按钮**，将这一系列视为一个「回复串」。
- **规则**：按**时间最早**的一条作为母 tweet（根），其余均标记为 reply；在数据中维护与母 tweet 的关系。
- **数据字段**（仅手动串中的条目会写入）：
  - **threadRootId**：该条所属串的根推文 id。根推文自身为 `threadRootId === id`；串内回复为根推文的 id。
  - **replyToId**：串内「直接回复」的那条 id。根推文无此字段；第一条回复为根 id；第二条回复为第一条的 id，形成链式顺序。
- **落定**：最后一次 Shift+点击后 2 秒无新操作则自动落定（排序、赋关系、写回存储）；若仅点击 1 条则仅保存，不形成串。
...
| 手动回复串 | `content.js` → Shift+点击走 `handleAddToThread(tweetElement)`，`threadBuffer` + 2s 后 `finalizeThread()`；存储合并见 `storage.js` → `saveTweet` 与已有条目合并 |

若目标网站改版导致选择器失效，可优先调整 `extractTweetInfo` 内对应步骤与 `addSaveButtonToTweet` 内操作栏查找方式，并同步更新本文档。

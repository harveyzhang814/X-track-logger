...
### Q: 推文类型（推文/回复/转推/引用）显示不对怎么办？

A: 类型是根据 X 页面上的 DOM 和文案自动识别的，界面改版或语言不同可能导致误判。若发现类型错误，反馈时请说明：页面场景（时间线/详情页/主页等）、您认为的正确类型、以及 X 的界面语言，便于我们迭代规则。详见 [故障排查指南](TROUBLESHOOTING.md#问题推文类型识别错误显示为推文回复转推引用与预期不符) 与 [逻辑说明](docs/逻辑说明.md)。
